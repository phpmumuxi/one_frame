...
$ko_count = 0;
foreach ($aaa as $vaa) {
    $vaa = trim($vaa); $a1 = get_dddate( $vaa );
    if (count($a1) < 1) {
        $a1 = get_dddate( str_replace('杂志', '', $vaa) );
    }
    if (count($a1) > 0) {
        foreach ($a1 as $v1) {
            preg_match('/<img src="(.*?)".*?>.*?<h4><a.*?href="(.*?)".*?>(.*?)<\/a><\/h4>/si', $v1, $r2);
        ...
        echo msg("关键词： ".$vaa."  - 采集完成\n"); //exit;
    } else {
        echo msg("关键词： ".$vaa."  - 没有相关符合的数据采集\n");
    }
}
echo msg("全部完成\n");


function get_dddate($name) {
    $s1 = Public_Http::curl( 'http://www.zazhi.com.cn/s.html?o=0&q='.urlencode($name).'&s=%E6%90%9C%E7%B4%A2' );
    preg_match_all('/<li class="list-li">(.*?)<div class="clear"><\/div>/si', $s1, $r1);
